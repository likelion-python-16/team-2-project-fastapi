...
PROJECT_ROOT: Path = APP_DIR.parent                     # 프로젝트 루트
STATIC_DIR: Path = APP_DIR / "static"                   # app/static
TEMPLATES_DIR: Path = APP_DIR / "templates"             # app/templates

# 파비콘 후보 경로 (존재 순서대로 시도)
FAVICON_CANDIDATES = [
    STATIC_DIR / "assets" / "full"  / "media" / "app" / "favicon.ico",
    STATIC_DIR / "assets" / "final" / "media" / "app" / "favicon.ico",
    # png 폴백
    STATIC_DIR / "assets" / "full"  / "media" / "app" / "favicon-32x32.png",
    STATIC_DIR / "assets" / "final" / "media" / "app" / "favicon-32x32.png",
]
...
async def signup_social_page():
    with open(TEMPLATES_DIR / "signup_social.html", "r", encoding="utf-8") as f:
        content = f.read()
...
async def public_user_profile(request: Request, user_id: int):
    # 통일: 새로운 마이페이지 템플릿 사용
    return templates.TemplateResponse("mypage/index.html", {"request": request, "view_user_id": user_id})
...
def verify_success_page():
    with open(TEMPLATES_DIR / "verify_success.html", "r", encoding="utf-8") as f:
        content = f.read()
    return HTMLResponse(content=content)

@app.get("/verify/fail", response_class=HTMLResponse, tags=["Email Verification"])
def verify_fail_page():
    with open(TEMPLATES_DIR / "verify_fail.html", "r", encoding="utf-8") as f:
        content = f.read()  
    return HTMLResponse(content=content)
