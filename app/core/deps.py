...
from app.security import verify_token, oauth2_scheme_optional

def get_current_user_from_cookie(
    request: Request,
    db: Session = Depends(get_db),
) -> Optional["User"]:
    """Parse JWT from HttpOnly cookie and return the user if valid.

    Tries `settings.auth_access_cookie_name` first, then falls back to 'session'.
    Returns None if missing/invalid without raising.
    """
    # local import to avoid circular
    from app.models.user import User

    cookie_name = settings.auth_access_cookie_name or "access_token"
    token = request.cookies.get(cookie_name) or request.cookies.get("session")
    if not token:
        return None
        ...
        if token:
            ...
            if payload:
                ...
                try:
                    ...
                    elif ident and hasattr(User, "email"):
                        user = db.query(User).filter(User.email == str(ident)).first()
